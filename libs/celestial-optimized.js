...
var ANIMDISTANCE = 0.035,  // Rotation animation threshold, ~2deg in radians
    ...
    ANIMINTERVAL_R = 1200, // Rotation duration scale in ms (優化: 從 2000 改為 1200)
  ...
  function rotate(config) {
    ...
    d3.select({}).transition().duration(interval).tween("center", function () {
      ...
      return function(t) {
        ...
        // 優化: 減少重繪頻率
        frameCount++;
        if (frameCount % skipFrames === 0 || t >= 0.99) {
          redraw();
        }
      };
  ...
  function redraw() {  
    ...
    if (cfg.constellations.lines) {
      container.selectAll(parentElement + " .constline").each(function(d) {
        // 檢查是否只顯示選中的星座
        var shouldShow = true;
        if (Celestial.selectedConstellation) {
          // 星座名稱映射：從完整英文名稱到縮寫ID
          var constellationMapping = {
            ...
            "Andromeda": "And", "Aquila": "Aql", "Ara": "Ara", "Auriga": "Aur",
            "Boötes": "Boo", "Caelum": "Cae", "Camelopardalis": "Cam",
            "Canis Major": "CMa", "Canis Minor": "CMi", "Carina": "Car",
            "Cassiopeia": "Cas", "Centaurus": "Cen", "Cepheus": "Cep", "Cetus": "Cet",
            "Columba": "Col", "Corona Australis": "CrA", "Corona Borealis": "CrB",
            "Corvus": "Crv", "Crater": "Crt", "Crux": "Cru", "Cygnus": "Cyg",
            "Delphinus": "Del", "Draco": "Dra", "Equuleus": "Equ", "Eridanus": "Eri",
            "Hercules": "Her", "Hydra": "Hya", "Lepus": "Lep", "Lupus": "Lup",
            "Lynx": "Lyn", "Lyra": "Lyr", "Monoceros": "Mon", "Ophiuchus": "Oph",
            "Orion": "Ori", "Pegasus": "Peg", "Perseus": "Per", "Piscis Austrinus": "PsA",
            "Puppis": "Pup", "Pyxis": "Pyx", "Sagitta": "Sge", "Serpens": "Ser",
            "Sextans": "Sex", "Triangulum": "Tri", "Ursa Major": "UMa", "Ursa Minor": "UMi",
            "Vela": "Vel", "Volans": "Vol", "Vulpecula": "Vul"
          };

          var selectedId = Celestial.selectedConstellation;
          var mappedId = constellationMapping[selectedId] || selectedId;

          // 檢查星座線條的ID是否匹配
          var lineId = d.id || d.properties.id;

          // 只顯示選中的星座線條（使用正確的ID字段）
          shouldShow = (d.id === selectedId ||
                       d.id === mappedId ||
                       (d.properties && d.properties.id === selectedId) ||
                       (d.properties && d.properties.id === mappedId));
        }
        ...
        if (shouldShow) {
          ...
          // 如果是選中的星座，加上發光效果
          if (Celestial.selectedConstellation && lineId === mappedId) {
            // 儲存原始設定
            var originalShadowColor = context.shadowColor;
            var originalShadowBlur = context.shadowBlur;
            var originalShadowOffsetX = context.shadowOffsetX;
            var originalShadowOffsetY = context.shadowOffsetY;

            // 多層發光效果
            var glowColors = ['rgba(0, 150, 255, 0.8)', 'rgba(0, 200, 255, 0.6)', 'rgba(100, 220, 255, 0.4)'];
            var glowSizes = [15, 10, 5];

            for (var i = 0; i < glowColors.length; i++) {
              context.shadowColor = glowColors[i];
              context.shadowBlur = glowSizes[i];
              context.shadowOffsetX = 0;
              context.shadowOffsetY = 0;

              setStyleA(d.properties.rank, cfg.constellations.lineStyle);
              // 增加線條寬度以增強效果
              context.lineWidth = (cfg.constellations.lineStyle.width || 1) * 2;
              map(d);
              context.stroke();
            }
            ...
            context.shadowOffsetX = originalShadowOffsetX;
            context.shadowOffsetY = originalShadowOffsetY;
    ...
    if (cfg.stars.show) {
      ...
      container.selectAll(parentElement + " .star").each(function(d) {
        if (clip(d.geometry.coordinates) && d.properties.mag <= cfg.stars.limit) {
          ...
          // 檢查是否為選中星座的星星
          var isSelectedConstellation = false;
          if (Celestial.selectedConstellation && window.starNamesData && window.starNamesData[d.id]) {
            var starData = window.starNamesData[d.id];
            var starConstellation = starData.c;
            var constellationMapping = {
              "Aries": "Ari", "Taurus": "Tau", "Gemini": "Gem", "Cancer": "Cnc",
              "Leo": "Leo", "Virgo": "Vir", "Libra": "Lib", "Scorpius": "Sco",
              "Sagittarius": "Sgr", "Capricornus": "Cap", "Aquarius": "Aqr", "Pisces": "Psc"
            };
            var mappedId = constellationMapping[Celestial.selectedConstellation] || Celestial.selectedConstellation;
            isSelectedConstellation = (starConstellation === mappedId);
          }

          // 如果是選中星座的星星，加上發光效果
          if (isSelectedConstellation) {
            // 儲存原始設定
            var originalShadowColor = context.shadowColor;
            var originalShadowBlur = context.shadowBlur;
            var originalShadowOffsetX = context.shadowOffsetX;
            ...
            // 多層發光效果
            var glowColors = ['rgba(255, 220, 100, 0.8)', 'rgba(255, 240, 150, 0.6)', 'rgba(255, 255, 200, 0.4)'];
            var glowSizes = [12, 8, 4];
            ...
            for (var i = 0; i < glowColors.length; i++) {
              ...
              context.beginPath();
              context.arc(pt[0], pt[1], r * 1.5, 0, 2 * Math.PI); // 稍微增大星星
              context.closePath();
              context.fill();
            }
